...
module.exports = function () {
	...
	this.mergeSettings = function (baseSettings, settings, alsoNullValue, onlyMatch) {
		// Verify if onlyMatch is undefined
		if (onlyMatch == undefined) onlyMatch = false;
		// Verify if onlyMatch is undefined
		if (alsoNullValue == undefined) alsoNullValue = false;
		...
		if (typeof settings == 'object') {
			...
			for (var i = 0; i < keys.length; i++) {
				...
				if (settings[key] !== undefined && (alsoNullValue || (!alsoNullValue && settings[key] !== null)) && (!onlyMatch || (onlyMatch && baseSettings[key] != undefined))) 
